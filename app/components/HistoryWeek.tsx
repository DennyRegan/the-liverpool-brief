...
export function HistoryWeek({ days }: { days: Days }) {
  return <div>
    {days.map(day => <section key={day.iso} className="history-section" aria-labelledby={`day-${day.iso}`}>
      <h2 id={`day-${day.iso}`}>
        <time dateTime={day.iso}>{day.label}</time><span>{day.weekday}</span>
      </h2>
      {day.events.length ? day.events.map(event => <article className="history-entry" key={event.slug}>
      ...
      </article>) : <p className="history-empty">No event selected.</p>}
    </section>)}
  </div>;
}
