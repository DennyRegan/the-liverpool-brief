...
export function ArticleCollection({ articles, home = false }: {
  articles: ArticleSummary[]; home?: boolean;
}) {
      ...
      <div aria-live="polite">
        {featured ? <>
          <article className="featured-article">
            ...
            <div>
              <p className="article-meta"><span>Article</span><span>{featured.date}</span></p>
          ...
          {rest.length > 0 && <section className="more-writing" aria-label="More articles"><p className="eyebrow">{home ? "Also recent" : "More writing"}</p><div className="article-grid">
            {rest.map(article => <article key={article.href} className="article-card"><p className="article-meta"><span>Article</span><span>{article.date}</span></p><h2><Link href={article.href}>{article.title}</Link></h2><p>{article.excerpt}</p><span className="reading-time">{article.minutes} min read</span></article>)}
          </div></section>}
