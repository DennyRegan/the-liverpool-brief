...
export default function MatchCentrePage() {
  ...
  const { last, next, ordered, upcoming, waiting } = selectMatches(data);
  ...
  function fixtureItem(f: Fixture) {
    return <li key={f.id} className="mc-fixture"><div className="mc-fixture-date">{f.date ? <time dateTime={f.date}>{dateLabel(f.date)}</time> : f.dateNote}<small>{f.status === 'completed' ? 'Full time' : f.status === 'postponed' ? 'Postponed' : f.status === 'cancelled' ? 'Cancelled' : waitingIds.has(f.id) ? 'Awaiting result' : fixtureTime(f)}</small></div><div><h4>{teams(f)}</h4><p>{competition(f.competitionId)} · {f.side === 'home' ? 'Home' : f.side === 'away' ? 'Away' : 'Neutral'}{f.round && ` · ${f.round}`}</p>{resultNote(f) && <p>{resultNote(f)}</p>}{f.date && f.dateNote && <p>{f.dateNote}</p>}{f.reportSlug && <Link href={`/archive/${f.reportSlug}`} className="read-link">Match report <span aria-hidden="true">→</span></Link>}</div></li>;
  }
  ...
  return <><SiteHeader active="match-centre" /><main id="main-content" className="site-width mc-page">
    ...
    <section className="mc-section" aria-labelledby="fixtures"><h2 id="fixtures">Fixtures &amp; Results</h2><p className="mc-note">Next four confirmed fixtures. Dates and kick-off times may change.</p>
      {upcoming.length > 0 ? <ul className="mc-upcoming" role="list">{upcoming.slice(0, 4).map(fixtureItem)}</ul> : <p className="mc-note">No upcoming fixtures have been confirmed.</p>}
      <details className="mc-disclosure"><summary>Show all fixtures and results <span className="mc-note">({ordered.length})</span></summary>
        <p className="mc-note">All recorded competitive fixtures. Further cup ties will be added after the draws.</p>
        {months.map(month => <section className="mc-month" key={month}><h3>{month === 'tbc' ? 'Date to be confirmed' : new Intl.DateTimeFormat('en-GB', { month: 'long', year: 'numeric', timeZone: 'Europe/London' }).format(new Date(`${month}-01T12:00:00Z`))}</h3><ul role="list">{ordered.filter(f => (f.date?.slice(0, 7) ?? 'tbc') === month).map(fixtureItem)}</ul></section>)}
      </details>
    </section>
    <section className="mc-section" aria-labelledby="league-table"><details className="mc-disclosure"><summary><h2 id="league-table">League Table</h2><span className="mc-note">Premier League · {data.season.replace('-', '–')} · As of {updatedLabel(data.table.asOf)}</span></summary><div className="mc-table-scroll"><table className="mc-table"><caption>Premier League standings, {data.season.replace('-', '–')}</caption><thead><tr><th scope="col"><abbr title="Position">Pos</abbr></th><th scope="col">Club</th><th scope="col"><abbr title="Played">P</abbr></th><th scope="col"><abbr title="Goal difference">GD</abbr></th><th scope="col"><abbr title="Points">Pts</abbr></th></tr></thead><tbody>{[...data.table.rows].sort((a,b) => a.position-b.position).map(r => <tr key={r.clubId} className={r.clubId === 'liverpool' ? 'mc-liverpool' : undefined}><td>{r.position}</td><th scope="row">{label(r.clubId)}{r.note && <small>{r.note}</small>}</th><td>{r.played}</td><td>{r.goalsFor-r.goalsAgainst > 0 ? '+' : ''}{r.goalsFor-r.goalsAgainst}</td><td>{r.points}</td></tr>)}</tbody></table></div><Sources data={data} ids={data.table.sourceIds} /></details></section>
