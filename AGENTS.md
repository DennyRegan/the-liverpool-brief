...
Requested factual drafts are permitted; explicit Denny approval is still required before publishing new articles. Drafting the next item does not depend on approval of the previous one. British English, text-only presentation, retrieved-source fact-checking, confidence labels in editorial notes, canonical metadata and existing article URLs remain mandatory. The full protocol, status/ownership rules and activation gate are in `docs/editorial/README.md`; no separate supporting-article queue is authoritative.
